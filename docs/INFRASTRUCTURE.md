...
The production stack creates a different private S3 bucket for PostgreSQL dumps under `postgresql/`. It has the same private/encrypted baseline, a configurable 90-day current-object retention period, 30-day noncurrent-version cleanup, and seven-day incomplete multipart-upload cleanup. The EC2 role can list that prefix and upload/read backup objects; it cannot make objects public or delete completed backups. Ansible manages the host-side backup helper and daily systemd timer, while S3 remains the durable storage and lifecycle authority. Scheduled backups use `postgresql/scheduled/`; manual migration safety backups use separate prefixes and are not modified by the scheduler.
