...
import { assignmentStatuses, priorities } from "@/lib/constants";
import { dateInputValue, formatDate, isBeforeToday, priorityClass, statusLabel } from "@/lib/utils";

export default async function AssignmentsPage({
  searchParams
}: {
  searchParams: Promise<{ course?: string; status?: string; priority?: string; edit?: string }>;
}) {
  ...
  return (
    <>
      <PageHeader title="Assignments" description="Plan coursework, deadlines, weight, and marks." />
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className={overdue ? "mt-2 text-sm font-medium text-red-600 dark:text-red-300" : "mt-2 text-sm text-muted-foreground"}>
                            ...
                            {assignment.marks !== null ? ` · Marks ${assignment.marks}` : ""}
                          </p>
                        </div>
    ...
    <Card>
      ...
      <CardContent>
        <form action={action} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Input type="number" step="0.1" name="weight" placeholder="Weight %" defaultValue={assignment?.weight ?? ""} />
            <Input type="number" step="0.1" name="marks" placeholder="Marks" defaultValue={assignment?.marks ?? ""} />
          </div>
